...
class RoomBookingFilter(django_filters.FilterSet):
    property = django_filters.NumberFilter(
        field_name="room__property__id", lookup_expr="exact"
    )
    booking_no = django_filters.CharFilter(
...
class EventSpaceBookingFilter(django_filters.FilterSet):
    ...
    status = django_filters.CharFilter(lookup_expr="icontains")
    event_space = django_filters.NumberFilter(
        field_name="event_space__id", lookup_expr="exact"
    )
...
class EventTicketFilter(django_filters.FilterSet):
    ticket_no = django_filters.CharFilter(
        field_name="ticket_number", lookup_expr="icontains"
    )
    status = django_filters.CharFilter(
        field_name="ticket_status", lookup_expr="icontains"
    )
