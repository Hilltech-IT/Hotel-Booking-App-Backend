...
def check_airbnb_date_conflict(airbnb, new_start, new_end, exclude_booking_id=None):
    bookings = BnBBooking.objects.filter(
        airbnb=airbnb, booked_to__gte=new_start, booked_from__lte=new_end
    )
...
def check_event_space_date_conflict(
    event_space, new_start, new_end, exclude_booking_id=None
):
