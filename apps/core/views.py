...
def home(request):
    ...
    airbnbs_revenue = sum(
        list(BnBBooking.objects.values_list("amount_paid", flat=True))
    )
    hotels_revenue = sum(
        list(RoomBooking.objects.values_list("amount_paid", flat=True))
    )
    events_spaces_revenue = sum(
        list(EventSpaceBooking.objects.values_list("amount_paid", flat=True))
    )
    events_revenue = sum(
        list(EventTicket.objects.values_list("amount_paid", flat=True))
    )
    ...
    if not user.is_superuser:
        airbnbs_count = (
            Property.objects.filter(owner=user).filter(property_type="AirBnB").count()
        )
        hotels_count = (
            Property.objects.filter(owner=user).filter(property_type="Hotel").count()
        )
        event_spaces_count = (
            Property.objects.filter(owner=user)
            .filter(property_type="Event Space")
            .count()
        )
        ...
        event_spaces_bookings = EventSpaceBooking.objects.filter(
        ...
        airbnbs_revenue = sum(
            list(
                BnBBooking.objects.filter(airbnb__owner=user).values_list(
        ...
        hotels_revenue = sum(
            list(
                RoomBooking.objects.filter(room__property__owner=user).values_list(
                    "amount_paid", flat=True
                )
            )
        )
        events_spaces_revenue = sum(
            list(
                EventSpaceBooking.objects.filter(event_space__owner=user).values_list(
                    "amount_paid", flat=True
                )
            )
        )
        events_revenue = sum(
            list(
                EventTicket.objects.filter(event__owner=user).values_list(
                    "amount_paid", flat=True
                )
            )
        )
